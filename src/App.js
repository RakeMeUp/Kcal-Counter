...
import Navbar from "./comps/newNavbar";
      ...
      <div className="flex flex-col">
        ...
        <header className="relative w-full flex justify-center py-6">
          <div className="flex justify-center w-full">
            <span className="text-2xl text-paletteGray font-medium ">
              Food List
            </span>
          </div>
        ...
        <main id="container" className="flex flex-col md:items-center">
          <ol>
            {this.state.listElements.length ? (
              this.state.listElements.map((listEl, index) => {
                return (
                  <ListElement
                    name={listEl.name}
                    kpp={listEl.kpp}
